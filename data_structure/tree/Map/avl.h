...
public:
	...
	void debug();
	void release();
	vector<pair<T,string>> sort();
	string first_elem();
	string last_elem();
private:
	node<T>* p, * head;
	vector<pair<T,string>> con;
	...
	pair<T,string> findMin(node<T>* root);
...
node<T>* AVL<T>::_erase(node<T>* root, T data) {
	...
	if (data < root->data) root->l = _erase(root->l, data);
		...
		if (root->l && root->r) {
			auto pack = findMin(root->r);
			root->data = pack.first;
			root->val = pack.second;
...
template<typename T>
pair<T,string> AVL<T>::findMin(node<T>* root) {
	while (root->l) root = root->l;
	return { root->data,root->val };
}
...
template<typename T>
vector<pair<T,string>> AVL<T>::sort() {
	con.clear();
	in_order(head);
	return con;
}
...
void AVL<T>::in_order(node<T>* root) {
	if (root) {
		in_order(root->l);
		con.push_back({ root->data,root->val });
		in_order(root->r);
	}
}
