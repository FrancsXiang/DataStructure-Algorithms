...
void limit_multi_pack() {
	...
	for (int i = 1; i <= n; i++) {
		...
		if (vol[i] * num[i] >= v) complete_pack(vol[i], val[i]);
			...
			//i.e. you can split 2^0...2^k and num[i]-2^k+1 to prove that the num(>=1&&<=num[i]) included will be visited by all parts's combination.
			int k = 1;
			while (k < num[i]) {
			...
			zero_one_pack(vol[i] * num[i], val[i] * num[i]);
		}
	}
}
